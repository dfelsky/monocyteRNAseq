# submit WGCNA for monocyte_blood
...
mono.resid <- readRDS("input/monocytes_techandagesexblood_residuals.rds")
...
pdf(file="output/WGCNA/mono_blood/subjectDendrograms.pdf",width=12,height=6)
print(plot_grid(dend1,dend2,ncol=2,labels = c("A","B")))
dev.off()

# save input dataset for WGCNA
saveRDS(datExpr,file="output/WGCNA/mono_blood/input_datExpr.rds")
...
pdf(file="output/WGCNA/mono_blood/softPower_study.pdf",width = 12,height=6)
...
##################### ONE STEP
##############################
datExpr <- readRDS("output/WGCNA/mono_blood/input_datExpr.rds")

setwd("output/WGCNA/mono_blood/")
