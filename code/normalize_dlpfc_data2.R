### generate normalized dlpfc gene expression data for WGCNA
library(limma)
library(edgeR)
...
outdir <- "/Users/dfelsky/Documents/monocyte_twas/integrated_analyses_v4/normalization/"

# load raw count and QC metric data from Hans and Annie's run
dlpfc_raw <- as.matrix(read.table("/Users/dfelsky/Documents/data/ROSMAP_RNAseq/dlpfc/from_CUMC_server_Nov112020/values/raw/geneCounts.txt",sep="",header=T,row.names = "gene_ID",check.names = F))
dlpfc_meta <- read.table("/Users/dfelsky/Documents/data/ROSMAP_RNAseq/dlpfc/from_CUMC_server_Nov112020/qualityControl/qualityMetrics.csv",sep="",header=T,colClasses = c(Sample="character"))
...
#saveRDS(dge,file=paste0(outdir,"dlpfc_raw.rds"))

### QC and forward confounder selection
source("/Users/dfelsky/Documents/monocyte_twas/integrated_analyses_v4/scripts/bulkRNA_QC.R")
dge <- readRDS(paste0(outdir,"dlpfc_raw.rds"))
...
fileConn<-file(paste0(outdir,"dlpfc_QCresults.txt"))
writeLines(
...
saveRDS(dge_filtered,file=paste0(outdir,"dlpfc_filtered_only.rds"))
...
saveRDS(resids,file=paste0(outdir,"dlpfc_techonlynorm_residuals.rds"))
saveRDS(rbeffect,file=paste0(outdir,"dlpfc_techonlynorm_removebatcheffectlogCPM.rds"))
saveRDS(resids.agesex,file=paste0(outdir,"dlpfc_techandagesex_residuals.rds"))
saveRDS(rbeffect.agesex,file=paste0(outdir,"dlpfc_techandagesex_removebatcheffectlogCPM.rds"))
saveRDS(v,file=paste0(outdir,"dlpfc_v.rds"))