...
dlpfc.resid <- readRDS("input/dlpfc_techandagesex_residuals.rds")

# cluster subjects and plot dendrogram prior to removal
datExpr <- t(dlpfc.resid)
...
manual_remove_subjects <- c("20634274","23690880","50103967","32697960","20886846","43596435","16322424","30544882","74522154","21246218","74284255","11390174","69924281","85578107","11475462","00402800","01797756","24141372")
dlpfc <- dlpfc.resid[,which(colnames(dlpfc.resid) %nin% manual_remove_subjects)]
datExpr <- t(dlpfc)
...
pdf(file="output/WGCNA/dlpfc/subjectDendrograms.pdf",width=12,height=6)
print(plot_grid(dend1,dend2,ncol=2,labels = c("A","B")))
dev.off()

# save input dataset for WGCNA
saveRDS(datExpr,file="output/WGCNA/dlpfc/input_datExpr.rds")
...
##################### ONE STEP 
##############################
datExpr <- readRDS("output/WGCNA/dlpfc/input_datExpr.rds")
...
net <- blockwiseModules(datExpr = datExpr,
                         ...
                         saveTOMs = T,
                         saveTOMFileBase = "dlpfc_signed_signed_minKMEtoStay03_noPAM",
                         pamStage = F,
                         pamRespectsDendro = F,
                         networkType = "signed",
                         verbose = 10)
...
saveRDS(net,"output/WGCNA/dlpfc/net_noPAM.rds")
net <- readRDS("output/WGCNA/dlpfc/net_noPAM.rds")

pdf(file="output/WGCNA/dlpfc/geneDendrogram.pdf", width = 12, height = 9)
...
pdf(file="output/WGCNA/dlpfc/genesPerModule_barPlot.pdf",width = 12,height=6)
