## sensitivity TWAS for monocytes including no covariates but same subjects as the runs with covariates. Use old design matrices
library(rms)
library(limma)
library(edgeR)
library(reshape2)
...
##################################
outcome.category.list <- c("cognition_noblood_sens")
tissue.list <- "monocyte"
...
for (tissue in tissue.list) {
  ...
  for (outcome.category in outcome.category.list) {
    ...
    # read in filtered data from normalize_mono_data.R
    # specify technical variables, covariates, and outcome variables
    if (tissue =="dlpfc") {
      ...
      if(outcome.category=="pathology"){
        ...
        } else if (outcome.category=="pathology_noblood_sens") {
          indepvec <- indepvec.pathology
          covars <- covars.mono.pathology.blood
          actual.covars <- covars.mono.pathology # this line allows for removal of co-variates from final design before lmfit
        } else if (outcome.category=="cognition_noblood_sens") {
          indepvec <- indepvec.mono.cognition
          covars <- covars.mono.cognition.blood
          actual.covars <- covars.mono.cognition # this line allows for removal of co-variates from final design before lmfit
        } }
    ...
    for (indep in indepvec) {
      ...
      if (indep %in% c("vm3123","pput3123","it3123","mf3123")){
        techvars <- techvars[-which(techvars=="study")]
      }
      ...
      designvars <- c(techvars,actual.covars,indep)  ### re-specify designvars so that the subject n is the same as if the full set of co-variates were included but the design only includes the original basic covariates.
